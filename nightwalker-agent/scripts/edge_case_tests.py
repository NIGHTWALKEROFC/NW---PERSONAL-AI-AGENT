...
Phase 15 (security audit + performance + scheduler) additions:
Telegram token redaction against a REAL requests exception (not a
synthetic string — proves the token would actually have leaked without
redaction); database/secret.key's file permissions are actually 0o600,
not just intended to be; the webhook server actually rejects an
oversized POST body with 413; the scheduler's full lifecycle including
a job that deliberately raises (proving one bad job doesn't crash the
scheduler or block other jobs); every default housekeeping job tested
against REAL rows that should and shouldn't be pruned (not just empty
tables — an overdue task, a future task, an expired session, a fresh
session implicitly, an old login attempt, an old consumed webhook row);
and the kill switch's scheduler integration end-to-end. Also see
test_candidate_generation_runs_concurrently_not_sequentially above,
which verifies the Phase 15 performance fix (parallelized candidate
generation) by actually measuring wall-clock time, not just checking
output correctness.
...
def test_empty_and_whitespace_candidates_are_skipped():
    ...
    # Phase 15: candidate generation now runs the 3 temperature calls CONCURRENTLY
    # (agent/reply/candidate_generator.py), so a plain side_effect=[list] mock is no
    # longer safe here — list-based side_effect consumption is not guaranteed
    # thread-safe, and more importantly nothing here should depend on call ORDER
    # anymore. Keying the fake response off the actual `options["temperature"]` each
    # call receives is correct regardless of which thread runs first.
    responses_by_temperature = {
        0.3: {"content": "  ", "elapsed_seconds": 0.1, "raw": {}},
        0.7: {"content": "a real reply", "elapsed_seconds": 0.1, "raw": {}},
        1.0: {"content": "", "elapsed_seconds": 0.1, "raw": {}},
    }

    def fake_chat(self, messages, stream=False, options=None):
        return responses_by_temperature[options["temperature"]]

    with patch("agent.brain.model_client.ModelClient.chat", fake_chat):
    ...
    if candidates:
        ...
        check("the surviving candidate kept its correct temperature", candidates[0]["temperature"] == 0.7)


def test_candidate_generation_runs_concurrently_not_sequentially():
    print("\n=== Candidate generator: the 3 temperature calls actually run concurrently, not one after another ===")
    import time
    from agent.reply.candidate_generator import generate_candidates

    call_delay_seconds = 0.3

    def slow_fake_chat(self, messages, stream=False, options=None):
        time.sleep(call_delay_seconds)
        return {"content": f"reply at temp {options['temperature']}", "elapsed_seconds": call_delay_seconds, "raw": {}}

    with patch("agent.brain.model_client.ModelClient.chat", slow_fake_chat):
        start = time.monotonic()
        candidates = generate_candidates([{"role": "system", "content": "x"}], "hi", "fake-model")
        wall_time = time.monotonic() - start

    check("all 3 candidates were generated", len(candidates) == 3, f"got {len(candidates)}")
    # Sequential would take ~3 * call_delay_seconds (~0.9s); concurrent should take
    # roughly one call's worth of time plus scheduling overhead. 2x a single call's
    # delay is a generous ceiling that still clearly fails if this silently regressed
    # back to sequential execution.
    check(
        f"wall time ({wall_time:.2f}s) is well under sequential time (~{3 * call_delay_seconds:.2f}s), confirming real concurrency",
        wall_time < call_delay_seconds * 2,
        f"took {wall_time:.2f}s",
    )
...
def test_kill_switch_revokes_dashboard_sessions():
    ...
    try:
        ...
        _reset_dashboard_auth_state()  # clean up — this test's data should not persist
        # Phase 15: reactivate() above also starts the scheduler as one of its side
        # effects (see agent/security/kill_switch.py) — stop it again so this Phase 14
        # test doesn't leak a running scheduler thread into whatever test runs next.
        scheduler.stop()


def test_telegram_token_redaction_against_a_real_exception():
    print("\n=== Security audit: Telegram bot token is actually scrubbed from a real request exception, not just in theory ===")
    import requests
    from connectors.telegram.telegram_adapter import _redact

    fake_token = "123456:ABC-DEF1234ghIkl-zyx57W2v1u123ew11"
    # A domain guaranteed not to resolve, so this reliably raises a RequestException
    # whose own __str__ embeds the full URL (and therefore the token) — exactly the
    # real-world case this redaction exists to protect against, not a synthetic string.
    url = f"https://this-domain-does-not-exist-nightwalker-edge-case.invalid/bot{fake_token}/getUpdates"
    try:
        requests.get(url, timeout=3)
        check("test setup produced a RequestException as expected", False, "request unexpectedly succeeded")
    except requests.exceptions.RequestException as e:
        raw = str(e)
        redacted = _redact(f"Telegram getUpdates failed: {raw}", fake_token)
        check("the raw exception really does contain the token (proves this is a meaningful test)", fake_token in raw)
        check("the redacted version no longer contains the token", fake_token not in redacted)
        check("the redacted version marks where it was removed", "[REDACTED_TOKEN]" in redacted)


def test_secret_key_file_has_restrictive_permissions():
    print("\n=== Security audit: database/secret.key is created with owner-only permissions, not world-readable ===")
    import stat
    from database.crypto import KEY_PATH, encrypt_text

    encrypt_text("trigger key creation if not already present")  # ensures the key file exists
    mode = stat.S_IMODE(os.stat(KEY_PATH).st_mode)
    check("secret.key permissions are exactly owner read/write (0o600)", mode == 0o600, f"got {oct(mode)}")


def test_webhook_server_rejects_oversized_payload():
    print("\n=== Security audit: an oversized webhook POST body is rejected before being fully read into memory ===")
    import os as _os
    from fastapi.testclient import TestClient

    _os.environ["INSTAGRAM_WEBHOOK_VERIFY_TOKEN"] = "edge-case-verify-token"
    _os.environ["INSTAGRAM_APP_SECRET"] = "edge-case-app-secret"

    from webhooks.webhook_server import app, MAX_BODY_BYTES
    client = TestClient(app)

    oversized_body = b"1" * (MAX_BODY_BYTES + 1000)
    resp = client.post("/webhooks/instagram", content=oversized_body, headers={"x-hub-signature-256": "sha256=irrelevant"})
    check("an oversized payload is rejected with 413, regardless of signature", resp.status_code == 413)


def test_scheduler_lifecycle_and_job_failure_handling():
    print("\n=== Scheduler: register/run/start/stop lifecycle, and a failing job doesn't crash the scheduler or other jobs ===")
    from agent.scheduler import scheduler

    # Defensive baseline: don't assume no earlier test left the scheduler running —
    # force a known-stopped starting point rather than asserting on inherited global
    # state, the same principle as _reset_dashboard_auth_state() elsewhere in this file.
    scheduler.stop()

    try:
        calls = []
        scheduler.register_job("edge_case_ok_job", lambda: calls.append("ok") or "did the thing", interval_seconds=9999)

        def failing_job():
            raise ValueError("deliberate failure for testing")

        scheduler.register_job("edge_case_failing_job", failing_job, interval_seconds=9999)

        ok_result = scheduler.run_job_now("edge_case_ok_job")
        check("a normal job reports success", ok_result["success"] is True and ok_result["result"] == "did the thing")

        fail_result = scheduler.run_job_now("edge_case_failing_job")
        check("a failing job reports success=False instead of raising out to the caller", fail_result["success"] is False)
        check("the failing job's error message is captured", "deliberate failure" in fail_result["error"])

        # Confirm the OTHER job is unaffected by the failing one
        ok_result_again = scheduler.run_job_now("edge_case_ok_job")
        check("a different job still runs fine after another job failed", ok_result_again["success"] is True)

        jobs_snapshot = {j["name"]: j for j in scheduler.list_jobs()}
        check("list_jobs shows the failing job's last_error", jobs_snapshot["edge_case_failing_job"]["last_error"] is not None)
        check("list_jobs shows the ok job's run_count incremented twice", jobs_snapshot["edge_case_ok_job"]["run_count"] == 2)

        unknown_result = scheduler.run_job_now("this_job_does_not_exist")
        check("running an unregistered job name returns an error instead of raising", "error" in unknown_result)

        check("scheduler is not running after the forced baseline stop() above", scheduler.is_running() is False)
        started = scheduler.start()
        check("start() returns True the first time", started is True)
        check("is_running() is True after start()", scheduler.is_running() is True)
        started_again = scheduler.start()
        check("calling start() again while running returns False (no duplicate thread)", started_again is False)

        was_running = scheduler.stop()
        check("stop() returns True when it was actually running", was_running is True)
        check("is_running() is False after stop()", scheduler.is_running() is False)
        was_running_again = scheduler.stop()
        check("calling stop() again when already stopped returns False", was_running_again is False)
    finally:
        scheduler.unregister_job("edge_case_ok_job")
        scheduler.unregister_job("edge_case_failing_job")
        scheduler.stop()  # clean up — this test's data should not persist


def test_default_housekeeping_jobs_actually_prune_the_right_rows():
    print("\n=== Scheduler jobs: each default housekeeping job prunes exactly the rows it should, and nothing else ===")
    import datetime
    from database.db import get_connection
    from agent.scheduler.jobs import (
        expire_overdue_tasks_job, prune_dashboard_sessions_job,
        prune_login_attempts_job, prune_webhook_inbox_job,
    )
    from database.task_store import create_task, list_tasks, delete_task
    from agent.security.dashboard_auth import set_password, create_session
    from database import webhook_inbox_store

    conn = get_connection()
    past = (datetime.datetime.utcnow() - datetime.timedelta(days=1)).isoformat() + "Z"
    future = (datetime.datetime.utcnow() + datetime.timedelta(days=1)).isoformat() + "Z"

    overdue_id = create_task("EDGE_CASE_OVERDUE_TASK_MARKER", expires_at=past)
    future_id = create_task("EDGE_CASE_FUTURE_TASK_MARKER", expires_at=future)
    try:
        expire_overdue_tasks_job()
        statuses = {t["id"]: t["status"] for t in list_tasks()}
        check("an overdue task gets marked expired", statuses[overdue_id] == "expired")
        check("a task with a future expiry is left untouched", statuses[future_id] == "pending")
    finally:
        delete_task(overdue_id)
        delete_task(future_id)

    try:
        _reset_dashboard_auth_state()
        set_password("edge-case-scheduler-job-pw")
        sid = create_session("Edge Case Old Device")
        conn.execute("UPDATE dashboard_sessions SET expires_at = ? WHERE session_id = ?", (past, sid))
        conn.commit()

        prune_dashboard_sessions_job()
        remaining = conn.execute("SELECT COUNT(*) AS c FROM dashboard_sessions WHERE session_id = ?", (sid,)).fetchone()["c"]
        check("an expired session is actually deleted by the pruning job", remaining == 0)
    finally:
        _reset_dashboard_auth_state()

    try:
        old_attempt_time = (datetime.datetime.utcnow() - datetime.timedelta(days=60)).isoformat() + "Z"
        conn.execute("INSERT INTO login_attempts (source_ip, attempted_at, success) VALUES (?, ?, 0)", ("203.0.113.200", old_attempt_time))
        conn.commit()
        prune_login_attempts_job()
        remaining = conn.execute("SELECT COUNT(*) AS c FROM login_attempts WHERE source_ip = '203.0.113.200'").fetchone()["c"]
        check("a 60-day-old login attempt is pruned (30-day cutoff)", remaining == 0)
    finally:
        conn.execute("DELETE FROM login_attempts WHERE source_ip = '203.0.113.200'")
        conn.commit()

    try:
        webhook_inbox_store.enqueue("instagram", "u1", "Old", "old message", "edge-case-prune-msg", "1")
        webhook_inbox_store.drain("instagram")  # mark consumed
        old_received_at = (datetime.datetime.utcnow() - datetime.timedelta(days=30)).isoformat() + "Z"
        conn.execute("UPDATE webhook_inbox SET received_at = ? WHERE message_id = 'edge-case-prune-msg'", (old_received_at,))
        conn.commit()
        prune_webhook_inbox_job()
        remaining = conn.execute("SELECT COUNT(*) AS c FROM webhook_inbox WHERE message_id = 'edge-case-prune-msg'").fetchone()["c"]
        check("an old CONSUMED webhook row is pruned", remaining == 0)
    finally:
        conn.execute("DELETE FROM webhook_inbox WHERE message_id = 'edge-case-prune-msg'")
        conn.commit()


def test_kill_switch_stops_and_reactivate_restarts_scheduler():
    print("\n=== Kill switch: activating it really stops the scheduler, and reactivating really restarts it ===")
    from agent.scheduler import scheduler
    from agent.security.kill_switch import activate, reactivate

    try:
        scheduler.start()
        check("scheduler is running before the kill switch", scheduler.is_running() is True)

        result = activate()
        check("activate() reports the scheduler was running and got stopped", result["scheduler_stopped"] is True)
        check("the scheduler is actually not running after activate()", scheduler.is_running() is False)

        reactivate(result["previous_levels"])
        check("the scheduler is actually running again after reactivate()", scheduler.is_running() is True)
    finally:
        scheduler.stop()  # clean up — this test's data should not persist


def test_resolve_approval_prevents_double_resolution():
    print("\n=== Security audit: resolving the same approval twice (double-click / two devices) is a no-op the second time ===")
    from agent.security.approval_queue import create_approval, resolve_approval, get_approval

    approval_id = create_approval("send_normal_reply", {"draft_text": "hello"}, reasoning="edge case test")
    try:
        first = resolve_approval(approval_id, "approved")
        check("the first resolution succeeds", first is True)

        second = resolve_approval(approval_id, "approved")
        check("resolving the SAME approval again returns False instead of silently succeeding", second is False)

        item = get_approval(approval_id)
        check("the approval's status is still just 'approved' (not double-processed)", item["status"] == "approved")
    finally:
        from database.db import get_connection
        conn = get_connection()
        conn.execute("DELETE FROM pending_approvals WHERE id = ?", (approval_id,))
        conn.commit()


def test_approvals_router_does_not_double_send_on_duplicate_approve():
    print("\n=== Security audit: hitting /approvals/approve twice for the same approval only sends once ===")
    from fastapi.testclient import TestClient
    from dashboard.app import app
    from agent.security.approval_queue import create_approval, get_approval
    from database.contact_store import get_or_create_contact, delete_contact

    contact_id = get_or_create_contact("EdgeCaseApprovalContact_DeleteMe")
    approval_id = create_approval(
        "send_normal_reply", {"draft_text": "double-send test"}, reasoning="edge case", contact_id=contact_id
    )
    try:
        send_calls = []

        def fake_dispatch_send(contact_id_arg, text):
            send_calls.append((contact_id_arg, text))
            return True

        with patch("dashboard.routers.approvals.dispatch_send", fake_dispatch_send):
            client = TestClient(app)
            r1 = client.post("/approvals/approve", data={"approval_id": approval_id}, follow_redirects=False)
            r2 = client.post("/approvals/approve", data={"approval_id": approval_id}, follow_redirects=False)

        check("both requests redirect successfully (no crash on the duplicate)", r1.status_code == 303 and r2.status_code == 303)
        check("the second response's message says it was already resolved", "Already+resolved" in r2.headers.get("location", ""))
        check("dispatch_send was called exactly ONCE despite two approve requests", len(send_calls) == 1, f"got {len(send_calls)} calls: {send_calls}")
    finally:
        from database.db import get_connection
        conn = get_connection()
        conn.execute("DELETE FROM pending_approvals WHERE id = ?", (approval_id,))
        conn.commit()
        delete_contact(contact_id)  # clean up — this test's data should not persist


def test_scheduler_run_now_route_distinguishes_success_from_not_found():
    print("\n=== Security/correctness audit: the scheduler's run-now route correctly distinguishes a successful run from a nonexistent job ===")
    from fastapi.testclient import TestClient
    from dashboard.app import app
    from agent.scheduler import scheduler
    ...
    try:
        with TestClient(app) as client:
            r_ok = client.post("/scheduler/jobs/expire_overdue_tasks/run-now", follow_redirects=False)
            location_ok = r_ok.headers.get("location", "")
            check("a real job's successful run reports 'succeeded', not the literal word 'None'", "succeeded" in location_ok and "None" not in location_ok, location_ok)

            r_missing = client.post("/scheduler/jobs/this_job_does_not_exist_edge_case/run-now", follow_redirects=False)
            location_missing = r_missing.headers.get("location", "")
            check("a nonexistent job name reports it wasn't found, not 'None'", "No" in location_missing and "job" in location_missing.lower(), location_missing)
    finally:
        scheduler.stop()  # clean up — the app's startup event started it; don't leak a running thread
...
def main():
    ...
    test_candidate_generation_runs_concurrently_not_sequentially()
    ...
    test_telegram_token_redaction_against_a_real_exception()
    test_secret_key_file_has_restrictive_permissions()
    test_webhook_server_rejects_oversized_payload()
    test_scheduler_lifecycle_and_job_failure_handling()
    test_default_housekeeping_jobs_actually_prune_the_right_rows()
    test_kill_switch_stops_and_reactivate_restarts_scheduler()
    test_resolve_approval_prevents_double_resolution()
    test_approvals_router_does_not_double_send_on_duplicate_approve()
    test_scheduler_run_now_route_distinguishes_success_from_not_found()
