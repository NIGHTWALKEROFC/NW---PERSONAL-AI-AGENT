...
def prune_consumed(older_than_days: int = 7) -> int:
    """
    Deletes CONSUMED rows older than the cutoff. Unconsumed rows are
    never touched regardless of age — those are messages nothing has
    read yet, and drain() already returns each row exactly once, so
    keeping consumed rows around indefinitely serves no purpose beyond
    duplicate detection, which only needs recent history in practice.
    Used by agent/scheduler/jobs.py's default housekeeping job (Phase 15).
    """
    conn = get_connection()
    cutoff = (datetime.datetime.utcnow() - datetime.timedelta(days=older_than_days)).isoformat() + "Z"
    cur = conn.execute("DELETE FROM webhook_inbox WHERE consumed = 1 AND received_at < ?", (cutoff,))
    conn.commit()
    return cur.rowcount
