...
def prune_expired_sessions() -> int:
    """
    Deletes revoked or expired session rows outright, rather than just
    leaving them to accumulate forever. list_active_sessions() already
    filters these out of what it shows, so this doesn't change any
    visible behavior — it just reclaims space. Used by
    agent/scheduler/jobs.py's default housekeeping job (Phase 15).
    """
    conn = get_connection()
    now = _now_str()
    cur = conn.execute("DELETE FROM dashboard_sessions WHERE revoked = 1 OR expires_at < ?", (now,))
    conn.commit()
    return cur.rowcount


def prune_old_login_attempts(older_than_days: int = 30) -> int:
    """Old login attempts (success or failure) beyond the lockout window's relevance
    are pure history — deleted after older_than_days so the table doesn't grow forever."""
    conn = get_connection()
    cutoff = (_now() - datetime.timedelta(days=older_than_days)).isoformat() + "Z"
