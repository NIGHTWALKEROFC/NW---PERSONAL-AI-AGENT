...
def resolve_approval(approval_id: int, decision: str, edited_payload: dict | None = None) -> bool:
    ...
    Returns True only if this call actually changed a PENDING approval to
    resolved. Returns False if the approval was already resolved (by an
    earlier call, a double-click, or — since Phase 14 — a second device
    acting on the same stale page) or doesn't exist. This is a Phase 15
    security-audit fix: the WHERE clause used to match on id alone, so
    resolving the same approval twice would silently succeed twice, and
    dashboard/routers/approvals.py would then call dispatch_send() twice
    for the same drafted reply — a real double-send risk, not a
    theoretical one, especially now that more than one device can be
    looking at the same Approval Center page at once.
    """
    ...
    if decision == "edited" and edited_payload is not None:
        cur = conn.execute(
            "UPDATE pending_approvals SET status = ?, payload = ?, resolved_at = ? "
