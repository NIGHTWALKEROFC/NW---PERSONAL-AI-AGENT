...
Phase 15 (security audit) fix: every endpoint here now checks
resolve_approval()'s return value before sending anything. It returns
False if the approval was already resolved — by an earlier click, a
double-submit, or (since Phase 14) a second device acting on the same
stale page — and in that case NOTHING is sent a second time. Before
this fix, a double-resolution would silently succeed twice and send
the same drafted reply twice.
"""
...
def get_router(templates) -> APIRouter:
    ...
    def approve(approval_id: int = Form(...)):
        ...
        if not resolve_approval(approval_id, "approved"):
            return RedirectResponse("/approvals?message=Already+resolved+%E2%80%94+nothing+sent+again.", status_code=303)
    ...
    def edit(approval_id: int = Form(...), new_text: str = Form(...)):
        if not resolve_approval(approval_id, "edited", edited_payload={"draft_text": new_text}):
            return RedirectResponse("/approvals?message=Already+resolved+%E2%80%94+nothing+sent+again.", status_code=303)
    ...
    def reject(approval_id: int = Form(...)):
        if not resolve_approval(approval_id, "rejected"):
            return RedirectResponse("/approvals?message=Already+resolved.", status_code=303)
