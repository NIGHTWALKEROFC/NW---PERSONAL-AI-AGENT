...
project.
*** BODY SIZE LIMIT (Phase 15 security audit addition) ***
This is the one endpoint in the whole project meant to be reachable
from the internet, which makes it the one place a stranger can send
arbitrarily large request bodies on purpose. Real Meta webhook
payloads are small (a handful of KB at most); MAX_BODY_BYTES below
rejects anything absurdly larger BEFORE it's read into memory or
JSON-parsed, so a malicious oversized POST can't be used to exhaust
memory.
"""
...
MAX_BODY_BYTES = 1_000_000  # 1 MB — generous for a real Meta payload, tight for an attack


async def _read_body_with_limit(request: Request) -> bytes | None:
    """
    Reads the request body in chunks, aborting the moment the total
    exceeds MAX_BODY_BYTES — this is enforced against the actual byte
    stream, not just the Content-Length header, since a header can be
    missing or simply wrong (chunked transfer encoding has no
    Content-Length at all). Returns None if the limit was exceeded.
    """
    total = 0
    chunks = []
...
async def receive_instagram_webhook(request: Request):
    raw_body = await _read_body_with_limit(request)
    if raw_body is None:
        log_event("webhook_body_too_large", "platform=instagram")
...
async def receive_whatsapp_webhook(request: Request):
    raw_body = await _read_body_with_limit(request)
    if raw_body is None:
        log_event("webhook_body_too_large", "platform=whatsapp")
        return Response(status_code=413, content="Payload too large")
